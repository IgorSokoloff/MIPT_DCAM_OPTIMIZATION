...
\begin{itemize}
\item Given $x0$, $B_0$(or $H_0$), $k \rightarrow 0;$
...
\begin{itemize}
	...
	$\min_{p\in \bb{R}^n} m_k(p) = f_k + \nabla f_k^Tp + \fr{1}{2}B_kp \quad ||p||\le \Updelta_k$
\end{itemize}
\medskip
\hrule\medskip
\Head{Numerical Results}
\begin{itemize}
	\item All quasi-newton methods with two strategies (8 algorithms) were implemented in Python
	\item 165 various $N-d (N \ge 2)$ strong benchmark problems
	\item For each algorithm all problems were launched from random point of domain 50 times and results were averaged
\end{itemize}

\begin{figure}
	\
	\includegraphics[width=.55\textwidth]{pic/NewFunction02.png}
	\includegraphics[width=.55\textwidth]{pic/CrossInTray.png}%\hfill
\end{figure}
\end{textblock}
...
\begin{figure}
	\includegraphics[width=.57\textwidth]{pic/line_search.pdf}
	\includegraphics[width=.57\textwidth]{pic/trust_region.pdf}
\end{figure}
